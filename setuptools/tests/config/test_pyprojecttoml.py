...
from setuptools.config.pyprojecttoml import (_InvalidFile, apply_configuration,
...
def create_example(path, pkg_root):
    ...
    (path / "{}/pkg/mod.py".format(pkg_root)).write_text("class CustomSdist: pass")
    (path / "{}/pkg/__version__.py".format(pkg_root)).write_text("VERSION = (3, 10)")
    (path / "{}/pkg/__main__.py".format(pkg_root)).write_text("def exec(): print('hello')")
...
class TestEntryPoints:
    ...
    def test_scripts_not_listed_in_dynamic(self, tmp_path, missing_dynamic):
        ...
        msg = "defined outside of `pyproject.toml`:.*{}".format(missing_dynamic)
...
def test_invalid_example(tmp_path, example, error_msg):
    ...
    pattern = re.compile("invalid pyproject.toml.*{}.*".format(error_msg), re.M | re.S)
